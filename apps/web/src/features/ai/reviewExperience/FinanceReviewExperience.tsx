/**
 * Phase 5 — Finance Review Experience connected to live import draft.
 */
...
import {
  applyLabeledFinanceOverviewEdit,
  financeOverviewCardModel,
  formatReviewDate,
...
import { FINANCE_SECTIONS } from "@/features/modules/manifests/reviewSections"
import { resolveModuleIcon } from "@/features/modules/moduleIcons"
import { Button } from "@/shared/ui/button"

const FINANCE_NAV: ReviewSectionItem[] = [
  ...FINANCE_SECTIONS.filter((s) => s.kind !== "images").map((s) => ({
    id: s.id,
    label: s.label,
...
export function FinanceReviewExperience({
  draft,
  saving,
  committed,
  regeneratingId,
  ...
  const [activeSectionId, setActiveSectionId] = useState(FINANCE_NAV[0]?.id ?? "session")
  ...
  const activeDef = FINANCE_SECTIONS.find((s) => s.id === activeSectionId)

  const centerCards: ReviewSectionCardModel[] = useMemo(() => {
    if (activeSectionId === "notebook") return []
    if (activeSectionId === "session") return [financeOverviewCardModel(draft)]
    if (!activeDef || activeDef.kind !== "content") return []

    const matched = resolveContentSections(activeDef, FINANCE_SECTIONS, draft.sections)
    if (!matched.length) {
      return [
        {
      ...
      header={
        <ReviewHeader
          model={{
            ...
            moduleName: "Finance",
            moduleIcon: Wallet,
            title: draft.title || "Untitled finance notes",
        ...
        activeSectionId === "notebook" ? (
          ...
          centerCards.map((card) => {
            ...
            const summaryValue = isSession
              ? financeOverviewCardModel(draft).summary
            ...
            return (
              <ReviewSectionCard
                ...
                onSummaryChange={(value) => {
                  if (isEmpty) return
                  if (isSession) {
                    onPatchDraft(applyLabeledFinanceOverviewEdit(value, draft))
                    return
                  }
...
export function FinanceReviewSuccess({ onContinue }: { onContinue: () => void }) {
  return (
    <div className="review-experience" data-module="finance">
      <div className="mx-auto flex max-w-md flex-col items-center gap-5 px-4 py-16 text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="re-body" style={{ color: "var(--re-muted)" }}>
            Your finance notes have been saved.
          </p>
        </div>
        <Button
          type="button"
